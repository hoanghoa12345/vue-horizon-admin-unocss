<template>
  <nav class="sticky top-4 z-40 bg-white dark:bg-[#0b14374d]">
    <div class="relative h-12 flex flex-row flex-wrap items-center justify-between gap-2 py-2">
      ...
      <input size="xl" type="text" :placeholder="t('search')" variant="outline" class="grow dark:text-white" />
        ...
        <transition enter-active-class="transition duration-100 ease-out"
          ...
          <MenuItems
            ...
            <div class="mt-3 ml-4 flex flex-col">
              ...
              <MenuItem v-slot="{ active }">
              <RouterLink to="/" class="mt-3 text-sm no-underline text-gray-800 dark:text-white hover:dark:text-white"
                :class="active ? 'font-bold' : ''">
                Newsletter Settings
              </RouterLink>
              </MenuItem>
